...
function log(msg) {
  console.log(`[dsh-aux-self-heal] ${msg}`);
}
...
function ensureSymlink(root) {
  ...
  if (brokenSymlink) {
    if (DRY) {
      log(`[dry-run] 将移除损坏 symlink 并重建: ${target} -> ${join(REPO, "dsh-aux")}`);
      return;
    }
    unlinkSync(target);
  ...
  if (DRY) {
    log(`[dry-run] 将重建 symlink: ${target} -> ${join(REPO, "dsh-aux")}`);
    return;
  }
...
function ensureSessionAppendIgnorable() {
  ...
  if (data.includes("opts[1]?.ignorable")) {
    log("P7 已打,跳过");
    return;
  }
  ...
  if (DRY) {
    log(`[dry-run] 将打 P7 session append ignorable(${label})`);
    return;
  }
  const bak = `${tgt}.bak-selfheal-${Date.now()}`;
  copyFileSync(tgt, bak);
  const out = data.replace(orig, patched);
  if (!out.includes("opts[1]?.ignorable")) {
    log("⚠️ P7 替换失败,已回滚");
    copyFileSync(bak, tgt);
    return;
  }
...
function ensureWhitelist(root) {
  ...
  for (const f of files) {
    ...
    if (data.includes('"aux/llm-call"') || data.includes("'aux/llm-call'")) {
      log(`P8 已含 aux/llm-call,跳过: ${f}`);
      continue;
    }
    const marker = "const KNOWN_SESSION_EVENT_TYPES = new Set([";
    if (!data.includes(marker)) {
      log(`⚠️ P8 无法自动补:未找到目录声明(${f})`);
      continue;
    }
    if (DRY) {
      log(`[dry-run] 将向白名单插入 aux/llm-call: ${f}`);
      continue;
    }
