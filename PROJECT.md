...
> 🔻 快照（2026-09-18）：包版本 `0.4.6-fix.1` · 主支 DSH `0.1.5-rc.2` · 测试基线 606。
...
- 当前版本：`0.4.6-fix.1`（`dsh-aux/package.json`）🔻 版本号与徽章随发布单独更新
- Node：>= 20
...
| 补丁族 | 作用 | 当前线 |
|---|---|---|
| P1-P6 / P11 | agent-loop / api-session-controller / skill schema | 必需 |
...
- 全量测试：`node --test tests/*.test.js`；🔻 基线 **606**（2026-09-18 快照，以实跑 `# pass/# fail` 为准，文件清单见 `TESTING.md`）。
