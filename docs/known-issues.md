...
- **状态**:暂缓,未修复(2026-09-03 确认;2026-09-09 复核仍成立)
...
- **涉及**:`dsh-aux/src/tools/vision.js`、`dsh-aux/src/index.js`(`_callRoute`)、`dsh-aux/src/route.js`(`classifyFailure` —— `INVALID_REQUEST` 归为 `other`);失败以结构化 `error: { code, message, retryable: false }` 加指令式文案返回,`other` 不做工具内自动重试
