...
function sha(n) {
  return "sha256:" + n.repeat(64).slice(0, 64);
}
...
test("D1: session-images 主文件损坏 → 从 .bak 恢复,保存不丢数据", async () => {
  ...
  try {
    ...
    assert.ok(
      files.some((f) => f.startsWith("session-images.json.corrupt-")),
      "损坏的主文件应被隔离保留: " + files.join(","),
    );
  } finally {
...
test("D2: 主 + 备份都损坏 → 两者都被隔离,不再静默丢进虚空", async () => {
  ...
  try {
    ...
    assert.ok(
      files.some((f) => f.startsWith("session-images.json.bak.corrupt-")),
      "备份损坏文件应隔离",
    );
  } finally {
    process.env.DSH_HOME = prev;
...
test("D3: image-memory 损坏 → 隔离后继续追加,新记录落盘", async () => {
  ...
  try {
    ...
    const files = await fsPromises.readdir(tmp + "/attachments/v1");
    assert.ok(
      files.some((f) => f.startsWith("image-memory.json.corrupt-")),
      "损坏 journal 应被隔离",
    );
  } finally {
    process.env.DSH_HOME = prev;
