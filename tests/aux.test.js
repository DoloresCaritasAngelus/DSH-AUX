...
test("resolveConfig: 空配置合法,未知键抛错", () => {
  assert.deepEqual(resolveConfig({}), {
    tasks: { vision: {}, web_extract: {}, web_crawl: {}, compress: {}, compaction: {}, skill: {} },
  });
...
test("resolveConfig: provider 与 model 必须成对", () => {
  assert.throws(
    () => resolveConfig({ tasks: { vision: { provider: "opencode-go" } } }),
...
test("resolveConfig: reasoningEffort 接受非空字符串,拒绝空/非字符串", () => {
  const resolved = resolveConfig({
    tasks: { vision: { provider: "opencode-go", model: "glm-5.2", reasoningEffort: "high" } },
  });
  assert.equal(resolved.tasks.vision.reasoningEffort, "high");
  assert.throws(
    () => resolveConfig({ tasks: { vision: { reasoningEffort: "" } } }),
    /reasoningEffort must be a non-empty string/,
  );
  assert.throws(
    () => resolveConfig({ tasks: { vision: { reasoningEffort: 42 } } }),
    /reasoningEffort must be a non-empty string/,
  );
});
...
test("resolvePrimaryRoute: 显式配置 > 任务默认 > undefined", () => {
  ...
  assert.deepEqual(resolvePrimaryRoute({ provider: "x", model: "y" }, defaults), route("x", "y"));
  assert.deepEqual(resolvePrimaryRoute({ task: "vision" }, defaults), route("opencode-go", "kimi-k2.7-code"));
...
test("htmlToText: 移除标签与脚本块,保留文本", () => {
  const html =
    '<html><head><title>忽略</title></head><body><script>var x = 1;</script><h1>标题</h1><p>正文内容 with <b>bold</b> and <a href="https://example.com">链接</a></p><style>.x{color:red}</style></body></html>';
...
test("url-policy: DNS 解析到内网地址时拒绝", async () => {
  ...
  await assert.rejects(
    () =>
      assertSafeFetchUrl("http://no-such-host.invalid/", {
        lookup: async () => {
    ...
    apply(stubCtx) {
      ...
      stubCtx.provide("web", {
        async fetch(request) {
          ...
          if (request.url.includes("error"))
            return { url: request.url, statusCode: 500, body: { kind: "text", content: "" }, truncated: false };
          if (request.url.includes("htmlpage"))
            return {
              url: request.url,
              statusCode: 200,
              body: {
                kind: "html",
                content:
                  "<html><body><script>var x=1;</script><h1>HTML 页面标题</h1><p>正文 with <b>bold</b> and number 42</p></body></html>",
              },
              truncated: false,
            };
          return {
            url: request.url,
            statusCode: 200,
            body: { kind: "text", content: "PAGE CONTENT here with facts and numbers 42" },
            truncated: false,
          };
        },
      });
      ...
      stubCtx.provide("fs", {
        async resolve(path, opts) {
          return { targetKey: path, displayPath: path, processPath: () => path, fileUrl: () => "file://" + path };
        },
        async stat(target) {
      ...
      stubCtx.provide("attachments", {
        ...
        async saveImage(input) {
          return {
            attachmentId: "att-" + input.mediaType,
...
function makeSession() {
  ...
  const session = {
    ...
    requestHeader() {
      return { config: { provider: "opencode-go", model: "deepseek-v4-flash" } };
    },
  };
  return session;
}
...
test("装配: ctx.auxLlm 可用,三工具注册,投影与命令注册", async () => {
  ...
  assert.ok(
    projections.some((p) => p.key === AUX_STATUS_KEY),
    "应注册 aux-status 投影",
  );
  assert.ok(
    projections.some((p) => p.key === AUX_PLATFORM_KEY),
    "应注册 aux-platform 投影",
  );
  assert.ok(
    projections.some((p) => p.key === AUX_IMAGE_LIBRARY_KEY),
    "应注册 aux-image-library 投影",
  );
...
test("装配: guideText 为空字符串时禁用引导段", async () => {
  ...
  assert.equal(
    sections.some((s) => s.name === "aux:tools-guide"),
    false,
    "空 guideText 不应注册引导段",
  );
});
...
test("Bootstrap 预设引导: minimal 目录过滤掉 AUX 工具,Anchored Standard 不过滤", async () => {
  ...
  assert.deepEqual(
    minimalPromotedNames,
    allTools.map((t) => t.name),
    "minimal 晋升后应开放 AUX 工具",
  );
...
test("Bootstrap 预设引导: pre-step 实际注入一次,晋升前不注入", async () => {
  ...
  assert.equal(
    before.messages.some((m) => m.source?.kind === "aux-guide"),
    false,
    "首轮不应注入 AUX 提醒",
  );
  ...
  assert.equal(
    after.messages.some((m) => m.source?.kind === "aux-guide"),
    true,
    "晋升后应注入 AUX 提醒",
  );
  ...
  assert.equal(
    second.messages.some((m) => m.source?.kind === "aux-guide"),
    false,
    "每个会话只注入一次",
  );
});
...
test("Bootstrap 预设引导: 首轮含图也不注入,晋升后才注入可用提醒", async () => {
  ...
  assert.equal(
    first.messages.some((m) => m.source?.kind === "aux-guide"),
    false,
    "首轮含图也不应注入 AUX 提醒",
  );
...
test("投影: aux/llm-call 事件折叠为每任务最近记录", async () => {
  ...
  const state = projection.apply(
    projection.apply(projection.init(), {
      type: AUX_CALL_EVENT,
...
test("call: fullToolTrace=true 时写入 aux/debug 事件", async () => {
  ...
  const session = makeSession();
  await ctx.auxLlm.call("compress", {
    messages: [
      {
        role: "user",
        content: [{ type: "text", text: "hello" }],
...
test("/aux debug: 展示当前会话 debug 事件", async () => {
  const session = {
    id: "sess-current",
    events: [{ seq: 1, type: AUX_DEBUG_EVENT, data: { kind: "call", task: "compress", ok: true, output: "OUTPUT" } }],
  };
  const service = {
    ctx: {
      get() {
              ...
              if (id === "sess-abc") {
                return {
                  events: [
                    {
                      seq: 1,
                      type: AUX_DEBUG_EVENT,
                      data: { kind: "call", task: "vision", ok: true, output: "VISION" },
                    },
                  ],
                };
              }
      ...
      return [
        {
          ...
          label: "会话压缩",
          configured: true,
          primary: { provider: "volcengine-ark", model: "doubao-seed-2.1-turbo" },
          timeoutMs: 60000,
          maxConcurrency: 2,
        },
      ];
...
test("compaction bridge: AUX 失败时抛出真实错误，不再 fallback 原生摘要", async () => {
  ...
  await assert.rejects(() => BasicCompactionEngine.prototype.summarize.call(fakeThis, input, agent), /AUX_REAL_ERROR/);
...
test("compaction bridge: 附件缺失的图片降级为文本占位,压缩仍可走 AUX", async () => {
    ...
    messages: [
      {
        ...
        content: [
          ...
          {
            type: "image",
            attachment: { attachmentId: "sha256:dead", mediaType: "image/png", width: 10, height: 10, name: "pic.png" },
          },
        ],
        id: "m1",
        source: { kind: "plugin", plugin: "test" },
  ...
  assert.equal(
    seen.messages[0].content.some((b) => b.type === "image"),
    false,
    "缺失图片不应再以 image block 发送",
  );
  ...
  await ctx.plugin({
    ...
    apply(stubCtx) {
      ...
      stubCtx.provide("attachments", {
        ...
        async validateImage() {},
        async saveImage(input) {
          return { attachmentId: "a", mediaType: input.mediaType, bytes: 0, width: 1, height: 1 };
        },
        async readImage(ref) {
      ...
      stubCtx.provide("agentDefaultModel", {
        currentSelection() {
          return { provider: "opencode-go", model: "main-ok" };
        },
      });
    },
  });
  ...
  return [
    {
      ...
      content: [
        {
          type: "image",
          attachment: { attachmentId: "att-1", mediaType: "image/png", bytes: 10, width: 5, height: 5 },
        },
    ...
    apply(stubCtx) {
      ...
      stubCtx.provide("llm", {
        async resolveModelInfo(provider, model) {
          return { provider, model, inputModalities: [] };
        },
        stream(options) {
          streams.push(options);
          return (async function* () {
...
test("vision_analyze 工具: 无 question(focus hint)时拒绝", async () => {
  ...
  await assert.rejects(() => tool.execute({ imagePath: "/tmp/test-image.png" }, exec), /question|required/i);
...
test("/aux status --json 命令: 返回结构化平台状态", async () => {
  ...
  for (const entry of data.items) {
    ...
    assert.ok(
      ["enabled", "disabled", "unavailable", "fixing", "unknown"].includes(entry.state),
      `state 非法: ${entry.state}`,
    );
...
test("web_extract 工具: 无 web provider 时回退直连传输并清洗 HTML", async () => {
  ...
  await ctx.plugin({
    ...
    apply(stubCtx) {
      ...
      stubCtx.provide("web", {
        async fetch() {
          throw new Error("no usable web provider is registered");
        },
      });
      stubCtx.provide("fs", {
        async resolve(p) {
  ...
  ctx.auxLlm._httpRequest = async (url) => ({
    ...
    text: async () =>
      "<html><body><h1>回退页面</h1><p>内容 with <b>bold</b> 42</p><script>var x=1</script></body></html>",
  });
  try {
...
test("/aux memory: 记录路径越界(无 DSH_HOME)时报错", async () => {
  ...
  try {
    ...
    if (prevHome === void 0) delete process.env.DSH_HOME;
    else process.env.DSH_HOME = prevHome;
    if (prevUserHome === void 0) delete process.env.HOME;
...
test("对账: 归档会话不误删(仍在持久化列表)", async () => {
  ...
  try {
    ...
    ctx.provide("sessionPersistence", {
      async listSnapshots() {
        return [{ header: { id: "sess-archived" } }];
      },
    });
    await reconcileSessionImages(ctx.auxLlm);
    const remaining = await fsPromises.readdir(objects);
...
test("vision_analyze 工具: images 数组并行分析多图,输出 analyses", async () => {
  ...
  const value = await tool.execute(
    {
      images: [{ imagePath: "/tmp/a.png" }, { imagePath: "/tmp/b.png" }, { imagePath: "/tmp/c.png" }],
      question: "每张图的主色调是什么?",
    },
    exec,
  );
...
test("vision_analyze 工具: images 数组超过 maxImagesPerMessage 时拒绝", async () => {
  ...
  await assert.rejects(() => tool.execute({ images, question: "q" }, exec), /maxImagesPerMessage/);
...
test("vision_analyze 工具: images 与单图参数互斥,条目必须恰有一个来源", async () => {
  ...
  await assert.rejects(() => tool.execute({ images: [{ nope: 1 }], question: "q" }, exec), /exactly one of/);
});
...
test("事件记录: 无 purpose 的调用(如 vision)写入的事件数据不含 undefined 字段", async () => {
  ...
  await ctx.auxLlm.call("vision", {
    messages: [
      { role: "user", content: [{ type: "text", text: "q" }], id: "m1", source: { kind: "plugin", plugin: "test" } },
    ],
    session,
    inputChars: 1,
  });
...
test("事件记录: sessionPatchCandidates 覆盖 symlink 与 realpath 两种部署布局", () => {
  ...
  assert.equal(realpath[1].href, "file:///x/dsh%20work/node_modules/@deepseek-ai/dsh-session/lib/index.js");
});
...
test("单级路由: 路由失败后回退主模型,事件记录候选与原路由", async () => {
  ...
  assert.equal(result.model, "deepseek-v4-flash", "唯一辅助路由失败后回退主模型");
  assert.deepEqual(calls, ["p/primary", "opencode-go/deepseek-v4-flash"], "只试辅助路由与主模型两条");
  const event = session.events.find((entry) => entry.type === AUX_CALL_EVENT);
  assert.deepEqual(event.data.candidates, ["p/primary", "opencode-go/deepseek-v4-flash"]);
  assert.equal(event.data.selectedIndex, 1);
  assert.equal(event.data.fallbackUsed, true);
});

test("冷却中的辅助路由被跳过 → 直接走主模型", async () => {
  const { ctx } = await makeHarness({ tasks: { compress: { provider: "p", model: "primary" } } });
  ...
  assert.equal(result.model, "deepseek-v4-flash");
  assert.deepEqual(calls, ["opencode-go/deepseek-v4-flash"], "冷却中的辅助路由不得发起调用");
});

test("describe(): 单级路由的来源与形状", async () => {
  const { ctx, commands } = await makeHarness({ tasks: { compress: { provider: "p", model: "single" } } });
  ...
  assert.equal(row.chain, void 0, "describe 不再输出整链");
  assert.equal(row.models, void 0, "describe 不再输出 models");
  const out = await commands[0].handler({ agent: void 0, rawInput: "status" });
  assert.equal(out.kind, "success");
  assert.ok(out.text.includes("p/single"), out.text);
  assert.ok(!out.text.includes("链:"), "status 不再输出整链");
});

test("/aux model: 写入单数 provider/model 并替换原路由", async () => {
  const { ctx, commands } = await makeHarness({ tasks: { compress: { provider: "p", model: "old" } } });
  const replaced = [];
