 ...
 * ——把 CHANGELOG 整文件替换成足量假 `## 0.x` 标题 + 哨兵、零正文
...
/** 发布段数的地板值 —— 从闸本身读,避免每次发版都要改测试里的魔数。 */
const RELEASE_BASELINE = Number(/CHANGELOG_RELEASE_BASELINE = (\d+)/.exec(readFileSync(SCRIPT, "utf8"))[1]);
...
/**
 * 构造假 CHANGELOG。
 * @param options.count `## 0.x` 段数(默认 = 闸里的地板值)。
 * @param options.withBody 段内是否带非空正文。
 * @param options.sentinel046 是否包含 v0.4.6 哨兵段。
 * @param options.unreleased 是否包含「未发布 (Unreleased)」节。
 */
function fakeChangelog({ count = RELEASE_BASELINE, withBody = true, sentinel046 = true, unreleased = true } = {}) {
  const parts = [];
...
test("ci-doc-hygiene: 发布段空正文(变异 B)→ 非零退出", () => {
  ...
  assert.notEqual(result.status, 0, "足量空壳段 + 哨兵必须阻塞(变异 B 不得漏网)");
