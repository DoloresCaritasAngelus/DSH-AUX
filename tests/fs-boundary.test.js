...
test('imagePath: 宿主 fs 拒绝时插件原样传播,不绕过沙箱', async () => {
  ...
  const fs = {
    ...
    async stat() { throw new Error('should not reach stat'); },
    async readBytes() { throw new Error('should not reach readBytes'); }
  };
  const attachments = {
    imageLimits: { maxImageBytes: 1000, maxMessageImageBytes: 1000 },
    async saveImage() { throw new Error('should not reach saveImage'); }
  };
  const service = makeService(fs, attachments);
  await assert.rejects(
    () => resolveImageRef(service, { imagePath: '/etc/passwd.png' }, exec),
    /fs sandbox denied/
  );
...
test('imagePath: readBytes 收到的是 fs.resolve 返回的 target,而不是原始用户路径', async () => {
  ...
  const readTargets = [];
  const fs = {
    async resolve() { return resolvedTarget; },
  ...
  const attachments = {
    imageLimits: { maxImageBytes: 1000, maxMessageImageBytes: 1000 },
    async saveImage(input) { return { attachmentId: 'att', mediaType: input.mediaType }; }
  };
  const service = makeService(fs, attachments);
