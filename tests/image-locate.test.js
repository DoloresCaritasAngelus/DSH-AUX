...
import { handleImageCommand } from "../dsh-aux/src/commands.js";
import { locateImageAnchors } from "../dsh-aux/src/images/locate.js";
...
function makeService({ sessions = {}, currentSession } = {}) {
  ...
  store.set("sessionPersistence", {
    ...
    inspect: async (sessionId) => {
      ...
      return session === void 0 ? { events: [] } : { meta: { id: sessionId }, events: session.events ?? [] };
    },
  });
  return {
    ctx: {
        ...
        "session-a": {
          ...
          events: [
            visionCallEvent({
              seq: 11,
              callId: "call_path",
              arguments: { imagePath: imagePathFor(id), question: "path?" },
            }),
            visionCallEvent({
              seq: 12,
              callId: "call_batch",
        ...
        "session-a": {
          ...
          events: [
            ...
            visionCallEvent({
              seq: 2,
              callId: "call_other",
              arguments: { attachmentId: attachmentIdFor("other"), question: "?" },
            }),
          ],
        },
