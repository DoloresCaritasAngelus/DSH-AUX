...
import { deleteImage, deleteOrphans, removeFromOwnership } from "../dsh-aux/src/images/image-actions.js";
...
test("deleteImage: referenced image without force throws REFERENCED", async (t) => {
  ...
  try {
    ...
    await assert.rejects(deleteImage(service, id), (error) => {
...
test("deleteOrphans: removes only orphan files; skips retained by default", async (t) => {
  ...
  try {
    ...
    const { file: retainedFile, extPath: retainedExt } = await fixture.writeObject(retainedId, {
      mediaType: "image/png",
      bytes: 24,
    });
