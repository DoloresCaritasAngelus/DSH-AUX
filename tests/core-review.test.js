  ...
  await ctx.plugin({
    ...
    apply(stubCtx) {
      ...
      stubCtx.provide('attachments', {
        ...
        async validateImage() {},
        async saveImage(i) { return { attachmentId: 'a', mediaType: i.mediaType, bytes: 0, width: 1, height: 1 }; },
        async readImage(r) { return { ref: r, data: new Uint8Array(0) }; }
      });
      stubCtx.provide('llm', { async stream() { throw new Error('no'); } });
      stubCtx.provide('sessionProjections', { register() { return () => {}; } });
      stubCtx.provide('commands', { register(def) { commands.push(def); return () => {}; } });
    }
  });
...
test('A2: no route and no main model records a no-route failure event before throwing', async () => {
  ...
  const session = { id: 'sess-review', events: [], append(type, data) { this.events.push({ type, data }); } };
