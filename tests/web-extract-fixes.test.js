...
import {
  ...
  readTextCapped,
  sniffMetaCharset,
} from "../dsh-aux/src/crawl/fetch-page.js";
  ...
  await withGlobalFetch(
    async (url) => {
      ...
      if (String(url) === "https://a.test/start")
        return { status: 301, url, headers: { get: () => "/mid" }, ok: false, text: async () => "" };
      if (String(url) === "https://a.test/mid")
        return { status: 302, url, headers: { get: () => "https://a.test/final" }, ok: false, text: async () => "" };
  ...
  await withGlobalFetch(
    async (url) => {
      ...
      return {
        status: 302,
        url,
        headers: { get: () => "http://127.0.0.1:3080/secret" },
        ok: false,
        text: async () => "",
      };
    },
...
test("H2: provider 返回缺 final URL 时拒绝(不信任事后缺失)", async () => {
  ...
  const harness = await withWebSeamHarness(async (s) => {
    s.provide("web", {
      async fetch() {
        return { statusCode: 200, body: { kind: "text", content: "x" } };
      },
    });
  });
  const exec = {
...
test("H2: provider 返回 3xx 时经本地逐跳重跟随(每跳 SSRF)", async () => {
  const harness = await withWebSeamHarness(async (s) => {
    s.provide("web", {
      async fetch() {
        return { url: "https://a.test/start", statusCode: 301, body: { kind: "text", content: "redirect stub" } };
      },
    });
  });
  ...
  await withGlobalFetch(
    async (url) => {
      ...
      return {
        ...
        url: String(url),
        headers: { get: () => "text/html" },
        text: async () => "<html><body>跟随后的最终页</body></html>",
      };
    },
    async () => {
      const exec = {
...
test("H2: provider 3xx 重跟随到内网在请求前被拒", async () => {
  const harness = await withWebSeamHarness(async (s) => {
    s.provide("web", {
      async fetch() {
        return { url: "https://a.test/start", statusCode: 301, body: { kind: "text", content: "" } };
      },
    });
  });
  let internalFetched = false;
  await withGlobalFetch(
    ...
    async () => {
      ...
      await assert.rejects(
        () => runWebExtract(harness.ctx.auxLlm, { url: "https://a.test/start" }, exec),
        /blocked by default/,
      );
...
test("H1: provider 抛 code=WEB_PROVIDER_UNAVAILABLE 回退本地", async () => {
  ...
  await withGlobalFetch(
    async (url) => ({
      ...
      text: async () => "FALLBACK CONTENT 42",
    }),
    async () => {
      const exec = {
        signal: new AbortController().signal,
      ...
      assert.ok(
        harness.streams[0].messages[0].content.find((b) => b.type === "text").text.includes("FALLBACK CONTENT 42"),
      );
    ...
    s.provide("web", {
      ...
      async fetch(request) {
        ...
        return {
          ...
          statusCode: 200,
          body: { kind: "text", content: "THIS BOUND OK 42" },
          truncated: false,
        };
      },
    });
...
test("H1 负路径: seam 抛自有校验错误(web provider returned…)不被吞、不回退本地", async () => {
  ...
  const harness = await withWebSeamHarness((s) => {
    s.provide("web", {
      async fetch() {
        throw new Error("web provider returned an invalid result for x");
      },
    });
  });
  await withGlobalFetch(
...
test("H2: seam 返回的最终 URL 指向内网 → 复审拒绝(post-check)", async () => {
  ...
  await assert.rejects(
    () => runWebExtract(harness.ctx.auxLlm, { url: "https://example.com/seam" }, exec),
    /blocked by default/,
  );
});
...
test("Low: 二进制 content-type 被拒绝", async () => {
  ...
  await withGlobalFetch(
    ...
    async () => {
      ...
      await assert.rejects(
        () => runWebExtract(ctx.auxLlm, { url: "https://example.com/a.png" }, exec),
        /not an HTML\/text page/,
      );
    },
  );
});
...
test("extractPageLinks: 同源过滤/相对解析/扩展名/去重/hash", () => {
  ...
  assert.deepEqual(links, ["https://a.example/page-a", "https://a.example/page-b", "https://a.example/docs"]);
});
...
test("F1 全流程: same-origin 递归抓取(本地路径)", async () => {
  ...
  const pages = {
    "https://a.example/root":
      '<html><body>ROOT TEXT<a href="/child">child</a><a href="https://other.example/x">skip</a></body></html>',
    "https://a.example/child":
  ...
  await withGlobalFetch(
    async (url) => {
      ...
      if (htmlText === undefined)
        return { ok: false, status: 404, headers: { get: () => "text/html" }, text: async () => "nf" };
    ...
    async () => {
      ...
      const value = await runWebExtract(
        ctx.auxLlm,
        { url: "https://a.example/root", followLinks: "same-origin", maxPages: 3, maxDepth: 2 },
        exec,
      );
...
test("F1: maxDepth=0 只抓根页;maxDepth 尊重层级", async () => {
  ...
  await withGlobalFetch(
    ...
    async () => {
      ...
      const value = await runWebExtract(
        ctx.auxLlm,
        { url: "https://a.example/root", followLinks: "same-origin", maxPages: 5, maxDepth: 0 },
        exec,
      );
...
test("F1: 共享 maxChars 预算截断累计文本", async () => {
  ...
  await withGlobalFetch(
    ...
    async () => {
      ...
      const value = await runWebExtract(
        ctx.auxLlm,
        { url: "https://a.example/root", followLinks: "same-origin", maxPages: 8, maxDepth: 1, maxChars: 6000 },
        exec,
      );
      assert.ok(value.totalChars <= 6000 + 40, "累计预算不超 maxChars(允许截断标记余量)");
      assert.ok(
        value.pages.some((p) => p.truncated),
        "存在被截断的页面",
      );
    },
  );
...
test("反爬: 无 header charset 时按 <meta charset> 解码(GBK 不乱码)", async () => {
  ...
  const bytes = Uint8Array.from([
    ...ascii('<html><head><meta charset="gb2312"></head><body>'),
    ...GBK,
    ...ascii("</body></html>"),
  ]);
  await withGlobalFetch(
...
test("反爬: 429 重试一次后成功,不再报错", async () => {
  ...
  await withGlobalFetch(
    async (url) => {
      calls += 1;
      if (calls === 1)
        return { ok: false, status: 429, url: String(url), headers: { get: () => "text/plain" }, text: async () => "" };
      return {
        ok: true,
        status: 200,
        url: String(url),
        headers: { get: () => "text/plain" },
        text: async () => "OK 429 重试成功",
      };
    },
    async () => {
      const exec = {
  ...
  await withGlobalFetch(
    async (url) => {
      ...
      if (u === "https://a.example/start")
        return { status: 301, headers: { get: () => "/mid" }, ok: false, text: async () => "" };
    ...
    async () => {
      try {
        ...
        const { response, finalUrl } = await fetchWithSsrf(
          { allowInternalUrls: false, _dnsLookup: async () => ({ address: "93.184.216.34" }) },
          "https://example.com/x",
          "web_extract",
        );
