...
import { detectNeedsVision, resolveSubagentRoute, AUX_SUBAGENT_TOOL_NAMES } from "../dsh-aux/src/subagent-route.js";
...
test("vision-aware + auto + 关键词 → vision", () => {
  const r = resolveSubagentRoute(visionAwareSettings, {
    prompt: "请描述这张图片 imagePath=/tmp/a.png",
    requiresVision: "auto",
  });
  assert.equal(r.settled, true);
  assert.deepEqual(r.agentOptions, { provider: "opencode-go", model: "kimi-k2.7-code" });
...
test("vision-aware 未配置 vision → native 兜底", () => {
  const r = resolveSubagentRoute(
    { mode: "vision-aware", general: { provider: "p", model: "m" } },
    { prompt: "看图", requiresVision: true },
  );
  assert.equal(r.settled, false);
});
...
test("vision-aware 缺少 general(且不需要视觉)→ settled=false", () => {
  const r = resolveSubagentRoute(
    { mode: "vision-aware", vision: { provider: "v", model: "vm" } },
    { prompt: "写代码", requiresVision: "auto" },
  );
  assert.equal(r.settled, false);
});
