        ...
        defineTool({
          ...
          parameters: {
            ...
            images: {
              type: "array",
              description:
                "Multiple images (or GIFs) to analyze in parallel with the SAME question. Each entry must be an object with exactly one of: attachmentId, imagePath, imageUrl.",
            },
            question: {
              type: "string",
              required: true,
              description:
                "The SPECIFIC thing you need to know about the image(s) (your intent). One focused question per call; ask a follow-up call for another detail.",
            },
          },
          output: {
            schema: {
  ...
  if (service.isToolExposed("web_extract")) {
    mountTool(
      ...
      defineTool({
        name: "web_extract",
        description:
          "Fetch a web page (or a same-origin set of pages via followLinks) and summarize it with the auxiliary model: returns a factual summary plus key points. Fetches static HTML only — no JavaScript rendering. Use when you need the essence of a page (or doc set) without carrying its full text.",
        parameters: {
          ...
          maxChars: {
            type: "integer",
            description:
              "Max page code points sent to the model (per call; for a crawl, the shared total budget). Default from config, else 32000.",
          },
          followLinks: {
            type: "string",
            description:
              "'off' (default): single page. 'same-origin': follow same-origin links, crawling up to maxPages pages within maxDepth and the shared maxChars budget.",
          },
          maxPages: { type: "integer", description: "Max pages to fetch when followLinks is same-origin (default 3)." },
          maxDepth: {
            type: "integer",
            description: "Max link depth to follow when followLinks is same-origin; 0 = root only (default 1).",
          },
        },
        output: {
          schema: {
            ...
            properties: {
              ...
              chars: {
                type: "integer",
                description:
                  "Code points of the page's retained content sent to the model (truncation marker excluded; single-page calls).",
              },
              truncated: { type: "boolean", description: "True when page text was cut to fit maxChars." },
              pages: {
                type: "array",
                description: "Per-page crawl metadata (present when followLinks is same-origin).",
                items: {
                  type: "object",
                  additionalProperties: false,
                  properties: {
              ...
              totalChars: {
                type: "integer",
                description:
                  "Total content code points sent to the model across a crawl (truncation markers/wrappers excluded).",
              },
              error: {
                type: "string",
                description: "Diagnostic notice when the page could not yield content (e.g. JS challenge).",
              },
              browserRequired: {
                type: "boolean",
                description:
                  "True when the page is a JS-challenge shell that needs a browser/rendering provider (no aux call was made).",
              },
              challengeProvider: {
                type: "string",
                description: "Detected challenge vendor (e.g. cloudflare) when browserRequired is true.",
              },
              httpStatus: { type: "integer", description: "HTTP status observed when a diagnostic is returned." },
              redirects: {
                type: "integer",
                description: "Redirect hops followed when known (local per-hop fetch path).",
              },
            },
          },
          render: (args, value) => [
  ...
  if (service.isToolExposed("web_crawl")) {
    mountTool(
      ...
      defineTool({
        name: "web_crawl",
        description:
          "Crawl a documentation site (or a whitelisted host set) starting from a seed URL and summarize the whole site with the auxiliary model: returns an overall summary plus per-page metadata. Respects robots.txt and per-host rate limits by default; every page and hop is SSRF-checked. Fetches static HTML only — no JavaScript rendering.",
        parameters: {
          ...
          scope: {
            type: "string",
            description:
              "'same-origin' (default): only the seed's origin. 'hosts': only the hostnames listed in hosts[] (domain scope is not enabled in v1).",
          },
          hosts: {
            type: "array",
            items: { type: "string" },
            description: "Allowed hostnames when scope is 'hosts' (the seed host must be included).",
          },
          ...
          maxCharsPerPage: {
            type: "integer",
            description: "Per-page code-point budget (default from config, else 32000).",
          },
          maxTotalChars: {
            type: "integer",
            description: "Total code-point budget across pages; 0 (default) derives it as maxPages × maxCharsPerPage.",
          },
          maxSeconds: { type: "integer", description: "Total time budget in seconds; 0 (default) = unlimited." },
          minIntervalMs: {
            type: "integer",
            description: "Minimum gap between requests to the same host (default 250).",
          },
          respectRobots: {
            type: "boolean",
            description: "Respect robots.txt (default true); set false to ignore robots checks.",
          },
          useSitemap: {
            type: "boolean",
            description:
              "Seed URL discovery from <origin>/sitemap.xml (default false); nested sitemap indices are skipped.",
          },
          seedUrls: {
            type: "array",
            items: { type: "string" },
            description: "Extra depth-0 seed URLs beyond the root seed; still SSRF-checked and scope-filtered.",
          },
          maxPagesPerHost: {
            type: "integer",
            description: "Max pages to fetch per host before skipping further links from it (0 = unlimited, default).",
          },
          perPageSummaries: {
            type: "boolean",
            description:
              "Produce a per-page summary (mode B, default false) instead of one aggregate summary; costs one auxiliary call per page plus one aggregation call.",
          },
          perPageConcurrency: {
            type: "integer",
            description: "Max concurrent per-page summary calls when perPageSummaries is true (default 1).",
          },
        },
        output: {
          schema: {
            ...
            properties: {
                ...
                items: {
                  ...
                  properties: {
                    ...
                    chars: {
                      type: "integer",
                      required: true,
                      description: "Retained content code points (marker excluded).",
                    },
                    truncated: { type: "boolean", required: true },
                    title: { type: "string" },
                  },
              ...
              totalChars: {
                ...
                required: true,
                description:
                  "Mode A: total content code points of crawled page texts; Mode B: total code points of per-page summaries.",
              },
              truncated: { type: "boolean", required: true },
              summary: { type: "string", required: true },
              keyPoints: { type: "array", items: { type: "string" }, required: true },
              ...
              error: {
                type: "string",
                description: "Diagnostic notice when the crawl yielded no content (e.g. JS challenge).",
              },
              browserRequired: {
                type: "boolean",
                description: "True when targets are JS-challenge shells needing a browser/rendering provider.",
              },
              challengeProvider: {
                type: "string",
                description: "Detected challenge vendor when browserRequired is true.",
              },
              httpStatus: { type: "integer", description: "HTTP status observed when a diagnostic is returned." },
  ...
  if (service.isToolExposed("compress_text")) {
    mountTool(
      ...
      defineTool({
        name: "compress_text",
        description:
          "Compress long text with the auxiliary model, preserving factual details (numbers, paths, identifiers). Use to shrink oversized tool output, research notes, or logs before they enter context.",
        parameters: {
          text: { type: "string", required: true, description: "The text to compress." },
          instruction: {
            type: "string",
            description: "Optional additional compression requirements (e.g. 'keep every file path').",
          },
          targetRatio: {
            type: "number",
            description: "Target compressed/original ratio (0.05-0.5). If maxOutputChars is set, it takes precedence.",
          },
          maxOutputChars: {
            type: "integer",
            description: "Optional output budget in characters. Preferred over targetRatio when provided.",
          },
          mode: {
            type: "string",
            description:
              "Optional soft compression profile hint: auto, code, log, doc, or general. Auto is recommended.",
          },
          preserve: {
            type: "array",
            items: { type: "string" },
            description:
              "Optional structured preservation hints: paths, numbers, headers, ids, urls, signatures, levels, stacktraces.",
          },
          hierarchical: {
            type: "boolean",
            description:
              "Optional deep compression for very large inputs (skeleton then refine). Usually auto-enabled above 200K chars.",
          },
        },
        output: {
          schema: {
            ...
            properties: {
              ...
              degraded: {
                type: "boolean",
                description: "True when at least one segment/round failed and original text was kept.",
              },
              warnings: {
                type: "array",
                items: { type: "string" },
                description: "Non-fatal warnings (e.g. unknown preserve hints, failed segment recovery).",
              },
            },
          },
