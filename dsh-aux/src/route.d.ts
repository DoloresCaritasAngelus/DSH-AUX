...
export declare function resolveConfig(config: unknown): {
  tasks: Record<string, AuxTaskConfigEntry>;
  guideText?: string;
  allowInternalUrls?: boolean;
};

/** Merge a settings section over plugin config (settings wins). */
export declare function mergeTaskConfig(
  pluginEntry: AuxTaskConfigEntry,
  settingsEntry: AuxTaskConfigEntry,
): AuxTaskConfigEntry;
 ...
 */
export declare function resolvePrimaryRoute(
  merged: AuxTaskConfigEntry & { task?: string },
  defaults: Record<string, AuxRoute>,
): AuxRoute | undefined;
...
export declare class FailureCooldown {
  ...
  snapshot(
    now?: number,
  ): Record<string, { provider: string; model: string; failures: number; coolingDown: boolean; until: number }>;
}
