...
![Tests](https://img.shields.io/badge/tests-617-brightgreen)
...
- **测试零依赖**：`node --test tests/*.test.js`（617 项；文件清单与基线见 `TESTING.md`）。
...
- **辅助路由与降级**：每个任务一条辅助路由（单数 `provider` / `model`，未配则回落到任务默认）。辅助路由失败后按 `fallbackToMain` / `visionFallbackToMain` 规则**回退到主模型**（视觉另需主模型显式声明支持图片，见下）。**不再有多级链**：需要跨多个模型备用时请自行改配置。
- **vision 打磨(Phase 3)**：失败条目给出原因与可否重试（限流/连接/服务端错误在工具内自动重试一次；**超时不重试** —— 路由已用光整个 `timeoutMs`，慢路由请调高该任务的超时）；`imagePath` 无扩展名按魔数嗅探（与 `read_image` 对齐）；动图只分析首帧；直连请求把校验通过的 IP 钉到连接（关闭 DNS rebinding）；`vision_analyze` 会话卡片显示 `【图N/共M】` 角标 + 缩略图 + 结论。
