...
- **移除误提交的 `node_modules` 符号链接**:该条目于上一次改动被带入仓库,内容是本机绝对路径 —— 一条指向维护者机器的符号链接,对使用者无用且泄漏环境信息。已删除,并补上当时漏检的那一类。
- **脱密闸改为扫描全部被跟踪文件**:原先按扩展名(`.md`/`.mjs`/`.js`/`.yml`/`.json`/`.sh`/`.txt`)挑选文件,于是**没有扩展名的文件根本不进扫描** —— 上面那条 `node_modules` 符号链接正是从这里漏过去的。现在判据改为内容:符号链接读**链接目标字符串**(符号链接的目标本身就是可能泄漏的路径),普通文件按 UTF-8 读、含 NUL 字节的判为二进制跳过(图片等既不误报也不会被解码坏)。

### 工程

- **私有台账引用不再写死路径**:仓库里有几处文档指向维护者的私有台账(该文件不随仓库分发,且其所在笔记库的目录结构由维护者调整)。写死路径会在外部结构变动时静默腐坏,现统一改为「补丁台账(私有,不随仓库分发)」的措辞。脱密闸自身的规则字面量保持不变。
