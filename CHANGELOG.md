...
## 0.4.6-fix.3 (2026-09-19) — 辅助模型思考块泄漏修复(reasoning 不得进入可见输出)

> **升级**:纯插件代码修复,**未改动桥接补丁**——重启 DSH 让新代码生效即可,不需要重打补丁、不需要改配置。唯一可感知的行为变化:辅助模型若某次只产出思考而没有正文,现在会判为失败并走既有降级链(主模型兜底;`skill` 预审回退原生结果),而不再把思考当作答案返回。

### 修复 — 思考内容被并入可见输出(影响全部 6 个辅助任务)

- **问题**:DSH 原生把模型输出分型为 `text` 与 `reasoning` 两个块类型,`ReasoningBlock` 的语义契约就是"distinct from visible text"。AUX 在唯一收口点 `_callRoute` 却把两者一起 `filter` 回来,再用 `join(" ")` 拼成一个字符串,于是**思考散文被当作正文返回**:
  - `vision_analyze` 的 `analysis`、`web_extract` / `web_crawl` 的 `summary` / `keyPoints`、`compress_text` 的 `compressed`、`skill` 预审报告全部可能混入思考;
  - `compaction` 的会话压缩摘要同样被污染,并**持久化进会话历史**;
  - 触发条件是任务路由开启了思考(`reasoningEffort` 非空),此时**每次调用必现**,并非偶发。
...
- **测试基线** 606 → 617(后经复核跟进再增至 625)。

### 修复 — bundle 接线的复核跟进(自愈不再覆盖用户开关等)

独立复核给出三条阻断,逐条修掉:

- **`ensureLegacyPatch` 会写出 DSH 拒绝的 YAML**:已有补丁文件是 `[]` 时追加块序列,得到 `[]` 紧跟 `- insert:` —— YAML 直接解析失败,真 DSH 报的正是本 PR 要消灭的那条 `overlay … must be a top-level YAML array`。修法:写入前先移走 `[]`,并在块前留一个空行(顺带避免后续摘除时把用户头注释一起吃掉)。
- **自愈会覆盖用户在插件页的「停用」**:插件页的开关只把包从 `dsh.profile.bundles` 移除、保留 `dependencies`,而自愈按「manifest 里提到本包」判定会把它回填 —— 等于每次启动都悄悄把用户关掉的东西打开。修法:把「依赖在、bundle 未选」认定为**用户停用态**,脚本与自愈一律保持现状(要恢复由用户在插件页打开)。
- **钉了 `DSH_ROOT` 却没给 `DSH_HOME` 时会写到真实 profile**:自愈此前只看 `DSH_HOME ?? ~/.dsh`,于是「用假部署根跑测试」会打到用户的真实 profile 目录。修法:钉了 `DSH_ROOT` 就必须同时给 `DSH_HOME`,否则跳过 profile 接线;镜像的 CI 假根已显式传入。
- **`doctor` 的假 OK**:它只查 patch 里有没有本包条目,不校验补丁层能否被 YAML 装载 —— 「只剩注释」这种真会被 DSH 拒绝的状态反而报 OK。修法:新增补丁层结构校验,四种状态(健康 / 只剩注释 / 两层同时存在 / 仍走 patch 注入)各有判定,已逐一实测。
- **摘条目的判据收紧**:原先按「整块子串含完整包名」判定,会误摘**别的插件**恰好提到 dsh-aux 的那条(生产补丁里 session-delete 的注释正是这种)。改为按条目自己的 `name:` 精确比对。
- **容器类型错不再静默**:`dsh: []` / `dependencies: []` / `bundles: {}` 这类错型此前会被 `JSON.stringify` 静默丢弃却报成功,现在一律报为问题且**不写盘**(`problem` 路径保证零副作用)。
- **profile 探测收紧**:只在注释里提到本包的 profile 不再被当成接线对象。
...
- **根因**:AUX 的 `vision_analyze` 工具视图声明子槽 `tool.call.images`,而官方 `dsh-client-ui-tool` 的 `read-image-toolview` 也声明同名子槽;一个槽名只允许一个声明者,后注册的一方注册被拒 —— `slot "tool.call.images" is already declared (by an entry in "tool.call.toolview" (read-image-toolview))`。
- **⚠️ 这个对撞不是 0.1.6 新增的**:0.1.5-rc.2 的 `dsh-client-ui-tool` 同样带 `read-image-toolview` 与同一行 `children: { "tool.call.images": … }`,`dsh-client-ui-slots` 同样带那条「只能声明一次」校验(两版逐字相同)。也就是说冲突在 rc.2 上**早已存在**,只是当时没有暴露成这个症状。
- **0.1.6 上为什么会整站不可进入,归因尚未查清**(候选:注册顺序使 AUX 成为落败的一方、错误上抛路径、启动审计)。一处反证值得记下:在一套干净的 0.1.6 上用**旧**客户端代码做对照,只复现出 console 里的同一条错误,界面**仍然挂载** —— 所以「0.1.6 必然整站不可进入」并不成立,真实触发条件与部署组合相关。
...
- **行为**:`vision_analyze` 行恢复完整渲染(角标 + 画廊 + 结论)。画廊点缩略图开原图(react-dom 可用时 portal 到 body,避免被变换过的祖先裁切;否则就地渲染);Esc / 点遮罩 / 关闭键都可关,而**点放大后的图不会关**(关闭处理挂在独立的遮罩层上,不挂在包含 `<img>` 的容器上 —— 挂在容器上会因冒泡把刚打开的预览关掉);加载失败可点击重试;`loadImage.peek` 缓存命中时首帧即出图,不再闪占位符。
- **为什么用私有前缀而不是去探测冲突**:槽名是全局命名空间,官方还在持续新增声明,而 `aux.` 前缀让这类撞名**在构造上不可能发生** —— 比任何检测都便宜,也不用维护一份官方槽名清单。
- **兼容性**:对 **0.1.5-rc.2 同样必要** —— 那边官方也声明同一个槽名,只是症状没暴露;私有前缀在两版上都不会与官方撞名。主支支持线仍为 **0.1.5-rc.2**。
- **测试基线** 600 → 606(新增 6 条:画廊注册/渲染 + 光箱开关、Esc、点大图不关、失败重试)。
...
- **移除的状态面**:`/aux status` 不再输出整链与「单数被忽略」警告;`describe()` 不再返回 `chain` / `models` / `singularIgnored`。**`aux/llm-call` 的 `candidates` / `selectedIndex` 仍然照写** —— 单级路由一样有候选集合(`[辅助路由]` 或 `[辅助路由, 主模型]`),`selectedIndex` 仍表示「谁作答」。
- **迁移**:`settings.yaml` 里残留的 `tasks.<task>.models` 会被**忽略**(不报错):合并与投影(`mergeTaskConfig` / `projectSettings`)都不再读它,该任务回到单数 `provider` / `model`,没有则回到任务默认、再回到主模型。注意**设置 schema 并不剥离未知键**,所以这个键会**留在文件里但永不生效**。三条路径行为不同:①settings 层 = 静默忽略;②**插件配置** = 报 `unknown key` 错误(那条路径本就严格校验);③`registerTask` 注册的自定义任务只校验 `key`,其 `models` 会**静默失效**(无报错无警告)。
...
- **支持线不变**:仍为 `0.1.5-rc.2`(CI compat 矩阵同此)。`0.1.2-alpha.2 ~ 0.1.2-rc.1` 用户请留在 `v0.4.4-legacy`。
- **补丁面有变更**:修复集中在 `dsh-agent-loop` 与 `dsh-api-session-controller` 两个宿主包,新增四个升级态(`gate-v3-upgrade` / `method-v3-upgrade` / `anchor-factual` / `nesting-upgrade`),**一律排在 `patched`(skip)判据之前** —— 带版本标记的 skip 判据只认标记、认不出语义缺口,排在后面等于让老部署永远停在旧行为上。
- **测试基线**:597 → **635**。

### 安全

- **移除误提交的 `node_modules` 符号链接**:该条目由一次工具改动带入仓库,内容是本机绝对路径 —— 对使用者无用,且泄漏环境信息。已删除。
- **脱密闸改为扫描全部被跟踪文件**:原先按扩展名(`.md`/`.mjs`/`.js`/`.yml`/`.json`/`.sh`/`.txt`)挑选文件,于是**没有扩展名的文件根本不进扫描** —— 上面那条符号链接正是从这里漏过去的。现判据改为内容:符号链接读**链接目标字符串**(链接目标本身即可能泄漏的路径),普通文件按 UTF-8 读、含 NUL 字节的判为二进制跳过(图片等既不误报,也不会被解码坏)。

### 修复

**平台开关的残留失效** —— 四处同一病根:**补丁与提示词没有跟随平台开关的演化**。

- **切 `imageBridge: native` 后,图片准入闸不再残留失效**:`dsh-api-session-controller` 的图片能力门控补丁此前**无条件删除**官方准入闸(不看平台开关),于是 `imageBridge` 切成 `native` 后 agent-loop 的改写停止、闸却仍处移除态 —— 纯文本主模型不再收到原生的明确拒绝,图片会直接流向 provider。现改为**条件生效**:桥接开启时让行,关闭(或插件未挂载)时官方闸重新生效。`image-bridge` 的补丁检测同时改为版本容忍,避免旧标记被误报为「未打补丁」。
- **图片锚点不再声称不可用的视觉工具**:桥接写入的锚点文本原先无条件写着「可用 `vision_analyze` 的 attachmentId 参数查看」;平台开关把该工具切成 `native` 后工具从模型目录消失,文案却照旧声称可用 —— 模型会去找一个不存在的工具。现改为向 AUX 询问工具的真实暴露度(`auxLlm.visionToolAvailable()`,委托既有的 `isToolExposed`);旧 AUX 或取值失败一律按可用处理,保持既有文本,不因升级而退化。
- **系统提示词不再宣传被开关关掉的工具**:`## 辅助模型工具(dsh-aux)` 一节与 Bootstrap 预步骤提醒原先无条件列出 `vision_analyze` / `web_extract` / `compress_text`。现按各工具的真实暴露度逐条生成:全部关掉时**整节不注入**,预步骤提醒为空时不发消息。
- **锚点回退文案改为纯事实**:上一项最初的回退串写作「(当前没有可用的视觉工具,无法查看此图)」—— 这同样是把一个此刻不存在的工具概念塞进上下文。现回退为**空串**,只保留「第 N/共 M 张 + attachmentId」这一事实。

**图片可达性** —— 改写只扫顶层,嵌套图片被漏掉。

- **桥接改为递归遍历嵌套图片**:原先只扫消息内容的顶层,于是 **`tool-result` 里嵌套的图片**(`read_image` 的结果就在那里)不被改道 —— 「forceAuxVision 成本路由」对它们失效(仍按原生视觉计费),纯文本主模型下它们也拿不到 `vision_analyze` 指引。官方把这种递归明确写成共享不变量(`contentHasImage` 的注释:「a consumer cannot silently diverge on nesting depth」),现与之一致并递归到任意深度;「本条消息第 N 张/共 M 张」的 M 同步改为递归计数。

**设置页诊断面板** —— 该面板自 `0.4.5` 起对所有部署失效。

- **「平台状态」面板恢复可用**:`aux/platform-status` 事件的载荷里,`imageLifecycle.blockedReason` 在**删除就绪**(即一切正常的常态)时是 `undefined`,而 DSH 拒绝写入含 `undefined` 的会话事件(`carries non-JSON-serializable data`);写入路径又把该异常**静默吞掉**,于是事件一条也写不出去、面板始终显示「无法获取平台状态」。原先的过滤只清**顶层** `undefined`,嵌套的照样进载荷 —— 现改为深度清理,并给写入路径补上可诊断输出(`DSH_AUX_DEBUG_PUBLISH=1` 时打印跳过或失败原因,默认静默)。
- **漏译文案与提示误报**:面板用拼键查表渲染,键缺失时会把**键名原样显示**(例如 `status.reason.force-aux-vision-overrides-route`);补齐 5 个缺失键(3 个 reason + 2 个 action,中英双语)并新增覆盖率闸,防止再次漂移。另外把「forceAuxVision 覆盖 visionRoute」这类**配置后果说明**从「需处理」里分出来:它是用户有意选择的一组配置,不是缺陷;计入待办会训练读者忽略面板。现以提示样式单独呈现并单独计数。

**网络**

- **代理回退只接受 HTTP(S) 端点**:DSH 拒绝 SOCKS 时会删掉 `HTTP(S)_PROXY` 且**不重写** `ALL_PROXY`,而 AUX 的代理回退读 `ALL_PROXY` 且不校验 scheme,会对 SOCKS 端点发 CONNECT 必然失败。现加白名单:`http:` / `https:` 之外一律视为无代理(回退直连)。

### 工程

- **补丁的平台开关约束闸**:`tests/bridge-switch-constraints.test.js` 为**每个补丁**钉住「切回 `native` 时如何让路」—— 读开关短路 / 委托会读开关的 AUX 服务 / 纯增量 schema(允许无条件,但必须证明原有行一行未少)。新增补丁必须在此登记其让路方式,防止上面那一族缺陷复发。
- **PR 描述纳入脱密闸**:脱密规则此前覆盖被跟踪文件与提交信息,而 **PR 标题与描述既不在工作树里、也不在提交历史里** —— 它同样进入公网,却是唯一没有闸的公开信道(规则齐备,信道没接上)。新增 `scripts/pr-body-hygiene.mjs`:扫当前分支的 PR(`--pr <n>`;`--stdin` 用于开 PR 前预检草稿),命中即非零退出。
...
- **规则校准**:原「私有工作区引用」规则把任意 `.local/` 前缀判为私有,但 `~/.local/share/dsh` 是 DSH 自身的安装位置候选,`$HOME/.local/bin` 是通用 CLI 安装位置 —— 二者都是正当的公开路径。该前缀已移除(绝对家目录路径仍由「本机绝对路径」规则覆盖),并补回归用例钉住这两个路径不得误报。
- **规模数字给出正确基准**:同一个脚本打印 `git diff --shortstat origin/main...HEAD`,并在本地 `main` 落后 `origin/main` 时告警 —— 本地引用陈旧时 `main...HEAD` 会把**已经合并的提交**一起算进去,数字虚高。
- **私有台账引用不再写死路径**:文档里有几处指向维护者的私有台账(该文件不随仓库分发,且其所在笔记库的目录结构由维护者调整)。写死路径会在外部结构变动时**静默腐坏**,现统一改为「补丁台账(私有,不随仓库分发)」的措辞。
- **`.gitignore` 忽略 `package-lock.json`**:CI 一律 `npm install --no-package-lock`,本仓库不维护锁文件,本地生成的产物不应被提交。
