...
- 🔻**易腐烂·快照数字** 基线 **628**(2026-09-18)。**以跑出的 `# pass/# fail` 为准**,
...
| `tests/profile-bundle.test.js` | profile bundle 接线(④):patch 条目精确移除(文件头注释与相邻条目不受伤)、`dependencies`/`bundles` 写入与幂等、dry-run 不写盘、全新部署的补丁兜底、profile 探测 |
